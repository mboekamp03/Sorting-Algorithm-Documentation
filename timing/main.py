...
t = lambda file: f"{TESTS_FOLDER}/{file}.csv"
...
def write_csv_test(test_name: str, cols: Mapping[str, str]):
    ...
    # Create a new map to ensure that no bs cols are added
    add_map = {key: cols[key] if key in cols else "" for key in test_csv_structure}

    # Open the file, and append
    with open(t(test_name), "a+") as file:
        file.seek(0)

        add_map["Test"] = sum(1 for _ in file.readlines())
...
RESULT_FILE = "Results"
result_csv_structure = ["Test Name", *[f"Mean (ms - {algo})" for algo in ALGORITHMS]]
...
def write_csv_result(cols: Mapping[str, str]):
    ...
    add_map = {key: cols[key] if key in cols else "" for key in result_csv_structure}
...
def compile_file():
    ...
    if not COMPILED_EXISTS:
        subprocess.call(["g++", "timeit.cpp", "sorts.cpp", "-o", COMPILED_NAME])
...
def perform_tests():
    # random helper
    rand = lambda x: [random.randint(100, 10_000) for _ in range(x)]
    ...
    for i in range(m):
        n, k = rand(2)
        print(f"Running {i}/{m}...")
        test_algorithm("Random-Outliers", random_array(n, k) + [random.randint(100_000, 1_000_000)], rep)
...
def analyze_test():
    ...
    tests = ["Correctly-Sorted", "Partial-Sorted", "Random-Sorted", "Reverse-Sorted", "Random-Outliers"]
